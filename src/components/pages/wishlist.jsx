...
const Wishlist = () => {
  ...
  async function addToWishList(productId, quantity) {
     try {
       ...
       if (!response.ok)
         return toast.error("Failed to load wishlist!!", { position: "bottom-center" });
       ...
       toast.error("Failed to load wishlist!!", { position: "bottom-center" })
     }
   }
  ...
  const total = subtotal + shipping;
    ...
    async function fetchWishlist() {
       try {
         ...
         if (!response.ok)
           return toast("Could not fetch WishList at the moment!!", { position: "bottom-center" });
   
         response = await response.json();
         console.log(response);
         setWishlistItems(response.message);
   
       } catch (error) {
        console.log(error)
         toast("Could not fetch WishList at the moment!!", { position: "bottom-center" })
       }
     }
        ...
        {wishlistItems.length === 0 ? (
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm">
            <div className="w-24 h-24 bg-pink-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <Heart size={42} className="text-pink-500" />
            </div>
            ...
            <button onClick={()=>navigator("/shop")} className="bg-[#7fad39] text-white px-8 py-4 rounded-2xl font-semibold">
              Continue Shopping
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
              ...
              {wishlistItems.map((item) => (
                ...
                >
                  ...
                  <div className="w-full md:w-40 h-40 rounded-2xl overflow-hidden">
                    <img
                      src={
                        import.meta.env.VITE_BACKEND_HOST +"/image/images/"+
                        item.image
                      }
                      alt={item.title}
            ...
            {/* <div>
              <div className="bg-white rounded-3xl p-8 shadow-sm sticky top-10">
                ...
                <button className="w-full mt-8 bg-[#7fad39] text-white py-5 rounded-2xl font-semibold flex items-center justify-center gap-2">
                  Add All To Cart
                  <ArrowRight size={20} />
                </button>

                <button className="w-full mt-4 border py-5 rounded-2xl font-semibold">
                  Continue Shopping
                </button>
